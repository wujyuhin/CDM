...
def cal_max_delta(q_list,item):
    ...
    for i in range(q_list.shape[0]):
        delta_list.append(cal_delta(q_m, item, q_list[i, :]))
    max_value = max(delta_list)
    max_index = delta_list.index(max_value)
    return max_value, max_index


def modify_qvector(q_m, item, epsilon=0.05):
    # 第一次迭代
    nums = []
...
for item in tqdm.tqdm(range(modify_q_m.shape[0])):
    modify_q_m = modify_qvector(modify_q_m,item)
# modify_q_m = modify_qvector(modify_q_m, item=10, epsilon=0.05)
...
#     deltas.append(cal_delta(q_m, item, one_q))
#     print(row)
#
